...
export default {
  ...
  methods: {
    ...
    syncFitbitSteps(awscredentials, botName, region) {
      return new Promise((resolve) => {
        const fitbitraw = localStorage.getItem('fitbitdata');
        localStorage.removeItem('fitbitdata');
        let completionCount = 0;
        if (fitbitraw) {
          const fitbitdata = JSON.parse(fitbitraw);
          const expectedCount = fitbitdata['activities-steps'].length;
          let aggregateData = [];
          let rawData = [];
          let curidx = 0;
          for (let i = 0; i < fitbitdata['activities-steps'].length; i++) {
            const item = fitbitdata['activities-steps'][i];
            const dt = moment(item.dateTime, 'YYYY-MM-DD');
            const value = item.value;
            this.updateReportedData(awscredentials, botName, region, dt, 'Steps', value)
              .then((data) => {
                completionCount += 1;
                aggregateData[curidx] = data.aggregate;
                rawData[curidx] = data.raw;
                curidx += 1;
                Logger.debug('curidx is ' + curidx);
                if (curidx > 0 || completionCount >= expectedCount) {
                  const dataToWriteRaw = rawData;
                  const dataToWriteAgg = aggregateData;
                  curidx = 0;
                  aggregateData = [];
                  rawData = [];
                  this.batchWriteItems(awscredentials, region, botName, 'Raw', dataToWriteRaw)
                    .then((dataWriteRaw) => {
                      Logger.debug('batchWriteItems raw: ' + JSON.stringify(dataWriteRaw, null, 2));
                      this.batchWriteItems(awscredentials, region, botName, 'Aggregate', dataToWriteAgg)
                        .then((dataWriteAgg) => {
                          Logger.debug('batchWriteItems aggregate: ' + JSON.stringify(dataWriteAgg, null, 2));
                          if (completionCount >= expectedCount) {
                            setTimeout(() => {
                              resolve('fitbitcomplete');
                            }, 1000);
                          }
                        });
                    });
          ...
          resolve('no fitbit data');
        }
      });
    },
    performUpdate(awscredentials, botName, region) {
      this.syncFitbitSteps(awscredentials, botName, region)
        .then((data) => {
          Logger.debug('sync from fitbit: ' + JSON.stringify(data, null, 2));
